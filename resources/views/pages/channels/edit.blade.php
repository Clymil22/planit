    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Edit Channel</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-1">Update channel information</p>
            </div>
            ...
            <form method="POST" action="{{ route('channels.update', $channel->id) }}">
                ...
                <div class="p-6 space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" required value="{{ $channel->name }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Description</label>
                        <textarea name="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ $channel->description ?? '' }}</textarea>
                    </div>
                    <div>
                        ...
                        <select name="type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            ...
                            <option value="project" {{ $channel->type === 'project' ? 'selected' : '' }}>Project</option>
                            <option value="support" {{ $channel->type === 'support' ? 'selected' : '' }}>Support</option>
                            <option value="private" {{ $channel->type === 'private' ? 'selected' : '' }}>Private</option>
                        </select>
                    </div>
                </div>
                <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 flex justify-end space-x-3">
                    ...
                    <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-violet-600 hover:bg-violet-700">Update Channel</button>
                </div>
            </form>
        </div>
    </div>
